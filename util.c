...
int dnslookup2(const char* hostname, char* IPstr){
    ...
    char ipstr[INET6_ADDRSTRLEN];
    int num_ipstrs = 0;
    char ipstrs[25][25 * sizeof(char) * INET_ADDRSTRLEN];
    int addrError = 0;
    ...
    for(result=headresult; result != NULL; result = result->ai_next){
		...
		if(result->ai_addr->sa_family == AF_INET){
			...
			int i;
			int duplicate = 0;
			for(i = 0; i < num_ipstrs; i++){
				if (strcmp(ipstrs[i], ipstr) == 0){
					duplicate = 1;
				}
			}
			if(!duplicate){
				strcpy(ipstrs[num_ipstrs], ipstr);
				num_ipstrs++;
			}
		}else if(result->ai_addr->sa_family == AF_INET6){
			/* IPv6 Handling */
